...
export function ChartRenderer({ visualization, height = 350 }) {
  ...
  const loadChartData = async () => {
    try {
      ...
      // Charger le dataset pour obtenir le chemin du fichier
      const datasetResponse = await fetch(`${API_BASE}/datasets/${datasetId}`, {
        headers: getAuthHeaders()
      });
      ...
      if (!datasetResponse.ok) {
        throw new Error(`Erreur chargement dataset: ${datasetResponse.status}`);
      }

      const dataset = await datasetResponse.json();
      console.log(dataset);
      
      const csvPath = dataset.path;
      console.log(csvPath);
      

      // Charger le CSV
      const csvUrl = csvPath.startsWith('http') ? csvPath : `${API_BASE}${csvPath}`;
      console.log(csvUrl);
      ...
      Papa.parse(csvText, {
        complete: (results) => {
          if (results.data && results.data.length > 1) {
            ...
            setCsvData(data);
            const mapped = mapChartData(data, visualization.config);
            setChartData(mapped);
            setLoading(false);
          } else {
